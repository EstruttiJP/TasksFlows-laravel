...
<div class="flex align-center justify-center">
    <div class="p-6 space-y-4 sm:p-8 bg-white rounded-lg shadow hover:shadow-lg transition-shadow w-full max-w-3xl">
        <h2 class="text-lg font-semibold">Update Task: {{ $task->name }}</h2>
        @if (session('status'))
        ...
        <form class="space-y-4" action="{{ route('tasks.update', $task->id) }}" method="POST">
            ...
            <div class="flex space-x-4">
                <div class="w-1/2">
                    ...
                    <input type="text" id="name" name="name"
                        ...
                        placeholder="Task Laravel" value="{{ old('name') ?? $task->name }}">
                    @error('name')
                ...
                <div class="w-1/2">
                    ...
                    <select id="status" name="status"
                        ...
                        <option value="In progress" @selected(old('status', $task->status ?? '') == 'In progress')>
                            In progress
                        </option>
            ...
            <div>
                ...
                <textarea id="description" rows="4" name="description"
                    ...
                    placeholder="Leave a description for Task...">{{ old('description') ?? $task->description }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="relative max-w-sm">
                ...
                <input id="deadline" type="text" name="deadline"
                    ...
                    placeholder="Select deadline" value="{{ old('deadline') ?? $task->deadline }}">
                @error('deadline')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="project_id" name="project_id"
                    ...
                    @foreach ($projects as $project)
                        <option value="{{ $project->id }}" @selected($project->id == old('project_id', $task->project_id))>
                            {{ $project->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <label for="departments" class="block mb-2 text-sm font-medium text-gray-900">Assign Users by
                    Department</label>
                @error('users')
                    <p class="mt-2 mb-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
                    ...
                    @foreach ($departments as $department)
                        <div class="bg-gray-100 p-4 rounded-lg shadow-md">
                            ...
                            <div id="users-{{ $department->id }}"
                                class="space-y-2 mt-2 overflow-auto max-h-56 hidden scrollbar-custom">
                                    ...
                                    @foreach ($department->users as $user)
                                        <label
                                            ...
                                            <input type="checkbox" name="users[]" value="{{ $user->id }}"
                                                class="form-checkbox text-primary-600" @checked(in_array($user->id, old('users', $task->users->pluck('id')->toArray())))>
            ...
            <button type="submit"
                class="float-end w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                Update
            </button>
        </form>
    </div>
