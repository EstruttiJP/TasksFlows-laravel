...
<div class="flex align-center justify-center">
    <div class="p-6 space-y-4 sm:p-8 bg-white rounded-lg shadow hover:shadow-lg transition-shadow w-full max-w-3xl">
        <h2 class="text-lg font-semibold">Department: {{$department->name}}</h2>
        @session('status')
        ...
        @endsession
        <form class="space-y-4" action="{{ route('departments.update', $department->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div>
                ...
                <div class="flex">
                    ...
                    <input type="text" id="name" name="name"
                        ...
                        placeholder="John Snow" value="{{ old('name') ?? $department->name }}">
                </div>
                @error('name')
